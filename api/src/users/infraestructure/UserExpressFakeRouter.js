...
router.post('/', withCatchExpressAsync(async (req, res) => {
  try {
    const params = {
      name: req.body.name,
    ...
    const response = await createUserHandler({ params, userRepository, userValidator, encrypter });

    return res.json(response);
  } catch (error) {
    console.log('Finalmente llegó aqui el error');
    throw error;
  }
}));
